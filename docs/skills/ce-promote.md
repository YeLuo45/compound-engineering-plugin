...
Spiral is detected into three states (`which spiral` + `spiral auth status`): ready, installed-but-unauthed, or absent. When ready, drafts are voice-matched to the user's brand and persist to their Spiral account (each draft carries a web-app `url` for tweaking). When not ready, the skill offers setup **once** — if installed but unauthed it runs `spiral login` and surfaces the sign-in link; if absent it points to the one-step install command — and a decline is always fine: it falls back to a lite layer of editorial & social-media expertise to draft strong copy on its own, and records the opt-out so it never nags again. The skill is equally useful with or without Spiral.
...
- **A real cross-channel set** → name the channels in the prompt; Spiral returns one draft per channel (and `--num-drafts` is ignored). One call produces the whole set.
...
**Cross-channel set:** `/ce-promote draft a launch across X, LinkedIn, and email` → (Spiral path) `spiral write "announcing one-click CSV export — a launch across X, LinkedIn, and email" --instant --json` returns one draft per channel; (no-Spiral path) the skill drafts one X post, one LinkedIn post, and one email directly. Each is labeled and ready to copy.
