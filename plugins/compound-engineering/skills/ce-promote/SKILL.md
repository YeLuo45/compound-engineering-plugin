...
- **Ready** (installed + authed) → **Path A** (voice-matched).
- **Unauthed** (installed, no key) or **absent** (no binary) → **Path 0** first, then Path A if the user sets it up, otherwise **Path B**.
...
- **Unauthed** → run `spiral login`, surface the link it prints, and let the user sign in by opening it. The user authenticates **themselves, in their own terminal** — never have them paste an API key into the chat (a key in the transcript leaks to the model and logs). Escape hatch: "or the agent can just draft directly, without Spiral's personalization and humanization." On success, re-run detection and use Path A.
...
Skip Path 0 entirely — straight to Path B — when the opt-out is already recorded, when running headless, or when no blocking-question tool is available.

### Path A — Spiral ready (voice-matched)

Use the Spiral CLI so drafts match the user's brand voice. Read `references/spiral-cli.md` for the exact invocation and the **multi-channel / cue-word gotcha** before composing the prompt — getting the phrasing wrong silently returns the wrong number or shape of drafts.

The short version:

- **Multi-channel set** (one draft per channel): phrase the prompt with the channels named — `spiral write "announcing <feature>: a tweet and a LinkedIn post" --instant --json`. `--num-drafts` is ignored in this mode; per-channel counts apply.
- **N variations of ONE channel**: ask for `"3 tweet options for <feature>"`, **avoid** the cue words `campaign`, `across`, `multi-channel`, `everywhere`, `cross-post` (they trip campaign mode and collapse to 1 draft), and pass `--num-drafts 3`.
- Always pass `--instant` (skips clarifying questions — this is headless) and `--json`. Parse `session_id`, `drafts[]` (each with `title`, `content`, `channel`, `url`).
...
> → Spiral path: `spiral write "announcing one-click CSV export — a launch across X, LinkedIn, and email" --instant --json` returns one draft per channel, each carrying its `channel`. (`--num-drafts` ignored here.) No-Spiral path: draft one X post, one LinkedIn post, one email directly. Present each labeled block.
